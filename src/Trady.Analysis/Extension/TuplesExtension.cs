...
namespace Trady.Analysis.Extension
{
    ...
    {
        public static IReadOnlyList<decimal?> Func<TInput>(this IEnumerable<TInput> inputs, Func<IReadOnlyList<TInput>, int, IReadOnlyList<decimal>, IAnalyzeContext<TInput>, decimal?> func, params decimal[] parameters) where TInput : IOhlcv => func.AsAnalyzable(inputs, parameters).Compute();

        public static IReadOnlyList<decimal?> Func<TInput>(this IEnumerable<TInput> inputs, string name, params decimal[] parameters) where TInput : IOhlcv => FuncAnalyzableFactory.CreateAnalyzable<TInput, decimal?>(name, inputs, parameters).Compute();
